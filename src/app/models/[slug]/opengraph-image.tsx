...
export default async function OgImage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const score = model.score?.overall == null ? "AA Benchmark Snapshot" : `Intelligence ${model.score.overall.toFixed(1)}`;
    ...
    (
      <div style={ogStyle()}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
          ...
          <div style={{ display: "flex", fontSize: 38, opacity: 0.9 }}>{score}</div>
        </div>
